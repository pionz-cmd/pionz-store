...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [orders, setOrders] = useState<any[]>(() => {
    try {
      const parsed = JSON.parse(localStorage.getItem('pionz_store_orders') || '[]');
  ...
  const [promoCodes, setPromoCodes] = useState<any[]>(() => {
    try {
      const parsed = JSON.parse(localStorage.getItem('pionz_promo_codes') || '[]');
      return Array.isArray(parsed) ? parsed : [];
    } catch { return []; }
  });
  ...
  const filteredAccounts = accounts.filter((a) => {
    ...
    const matchesSearch = String(a.title || '').toLowerCase().includes(q) || String(a.code || '').toLowerCase().includes(q) || String(a.category || '').toLowerCase().includes(q) || String(a.loginType || '').toLowerCase().includes(q);
