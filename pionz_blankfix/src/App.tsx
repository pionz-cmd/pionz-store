...
export default function App() {
    ...
    if (saved) {
      try {
        ...
        if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
          ...
          return { ...initialConfig, ...safeConfig };
        }
      } catch (e) {
        console.error(e);
      }
  ...
  // No-database mode: catalog and configuration persist locally in the browser.
  ...
  const handleSaveAccount = (accountToSave: FFAccount) => {
    setAccounts((prev) => {
      const i = prev.findIndex((a) => a.id === accountToSave.id);
      const next = i >= 0 ? prev.map((a, idx) => idx === i ? accountToSave : a) : [accountToSave, ...prev];
      localStorage.setItem('pionz_store_accounts', JSON.stringify(next));
      return next;
    });
  };
  ...
  const handleSaveConfig = (newConfig: StoreConfig) => {
    setConfig(newConfig);
    localStorage.setItem('pionz_store_config', JSON.stringify(newConfig));
  };
