...
import { trackEvent, trackSessionVisit } from './utils/analytics';
import {
  ShieldCheck,
  Search,
  Sparkles,
...
export default function App() {
  // Anonymous storefront analytics (no names, passwords, or raw IPs are stored).
  useEffect(() => {
    trackSessionVisit();
  }, []);
  ...
  // Production sync: products/config live in Supabase through the Vercel API.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (Array.isArray(data.accounts) && data.accounts.length) setAccounts(data.accounts);
        else if (initialAccounts.length) {
          const sr = await fetch('/api/store', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'seed', accounts:initialAccounts}) });
          if (sr.ok) setAccounts(initialAccounts);
        }
        if (data.config) setConfig(data.config);
      } catch (e) { console.warn('Database sync unavailable; using local fallback.', e); }
    })();
  ...
  // Login logs remain a local UI fallback; product/config data are server-backed.
  useEffect(() => {
  ...
  const handleSaveAccount = (accountToSave: FFAccount) => {
    setAccounts((prev) => { const i=prev.findIndex(a=>a.id===accountToSave.id); if(i>=0){const n=[...prev];n[i]=accountToSave;return n;} return [accountToSave,...prev]; });
    fetch('/api/store',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'upsert_account',account:accountToSave})}).catch(console.error);
  };

  const handleDeleteAccount = (id: string) => {
    setAccounts((prev) => prev.filter((a) => a.id !== id));
    fetch('/api/store',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete_account',id})}).catch(console.error);
  };

  const handleToggleStatus = (id: string, newStatus: AccountStatus) => {
    setAccounts((prev) => prev.map((a) => a.id === id ? {...a,status:newStatus} : a));
    const account=accounts.find(a=>a.id===id); if(account) fetch('/api/store',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'upsert_account',account:{...account,status:newStatus}})}).catch(console.error);
  };

  const handleSaveConfig = (newConfig: StoreConfig) => {
    setConfig(newConfig);
    fetch('/api/store',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'save_config',config:newConfig})}).catch(console.error);
  };
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && searchQuery.trim()) {
                    void trackEvent('search', { value: searchQuery.trim().slice(0, 120) });
                  }
                }}
          ...
          {[
            ...
            <button
              ...
              onClick={() => {
                ...
                void trackEvent('category_filter', { value: cat.key });
              }}
            ...
            {sortedAccounts.map((account) => (
              <AccountCard
                ...
                onViewDetail={(acc) => {
                  ...
                  void trackEvent('account_view', { accountId: acc.id, accountCode: acc.code });
                }}
              />
            ))}
          </div>
