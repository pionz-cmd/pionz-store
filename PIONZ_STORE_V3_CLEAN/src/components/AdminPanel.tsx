...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [newUsername, setNewUsername] = useState(config.adminUsername);
  const [newPassword, setNewPassword] = useState(config.adminPassword);
  const [newSecurityKey, setNewSecurityKey] = useState(config.securityKey);
  ...
  const handleSaveStoreConfig = (e: React.FormEvent) => {
    ...
    const finalConfig: StoreConfig = {
      ...tempConfig,
      adminUsername: newUsername.trim() || config.adminUsername,
      adminPassword: newPassword.trim() || config.adminPassword,
      securityKey: newSecurityKey.trim() || config.securityKey,
    };
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6">
            ...
            <div className="space-y-5 max-w-3xl text-xs">
              ...
              <div className="bg-[#070b14]/80 p-4 rounded-xl border border-sky-500/30 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs">
                  ...
                  <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800">
                    <span className="text-slate-400 block text-[10px]">Pemulihan Darurat:</span>
                    <span className="font-bold text-white">Security Key Enkripsi</span>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'settings' && (
            <form onSubmit={handleSaveStoreConfig} className="space-y-4 max-w-2xl text-xs">
              ...
              <div className="p-4 bg-sky-950/20 border border-sky-500/30 rounded-xl space-y-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">
                    Gunakan kode ini jika sewaktu-waktu Anda lupa password admin.
                  </p>
                </div>
              </div>
